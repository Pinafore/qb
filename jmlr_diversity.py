...
def fetch_parse(sentence):
    ...
    properties = json.dumps(
        {"annotators": "tokenize,ssplit,parse", "date": "2018-09-12T16:34:00"}
    )
...
for depth in tqdm.tqdm_notebook(range(1, deepest_parse_depth)):
    ...
    qb_linear, qb_lengths = parses_to_linears(
        linearize_parse, qb_parse_results, qb_sentences
    )
    sq_linear, sq_lengths = parses_to_linears(
        linearize_parse, sq_parse_results, sq_questions
    )
    squad_linear, squad_lengths = parses_to_linears(
        linearize_parse, squad_parse_results, squad_questions
    )
    tqa_linear, tqa_lengths = parses_to_linears(
        linearize_parse, tqa_parse_results, tqa_questions
    )
    jeopardy_linear, jeopardy_lengths = parses_to_linears(
        linearize_parse, jeopardy_parse_results, j_questions
    )

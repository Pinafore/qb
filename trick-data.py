...
def merge():
    ...
    verify_checksum(
        trickme_id_model_checksum, "data/external/datasets/trickme-id-model.json"
    )
    with open("data/external/datasets/trickme-id-model.json") as f:
        id_to_model = json.load(f)
        id_to_model = {int(k): v for k, v in id_to_model.items()}
    ...
    verify_checksum(
        qanta_expo_checksum, "data/external/datasets/qanta.trick-no-edits.json"
    )
    with open("data/external/datasets/qanta.trick-no-edits.json") as f:
        data = json.load(f)
    ...
    verify_checksum(
        additional_ir2_checksum,
        "data/external/datasets/qanta.trick-additional-ir-round2.json",
    )

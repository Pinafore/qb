...
if [ "$FEATURE" = "t" ]
then
    vw --compressed -d data/vw_input/dev.sentence.${WEIGHT}.vw_input.gz --early_terminate 100 -k --keep g --keep t -q gt -b 28 --loss_function logistic -f data/models/sentence.full.${WEIGHT}.t.vw
...
then
    ...
    vw --compressed -d data/vw_input/dev.sentence.${WEIGHT}.vw_input.gz --early_terminate 100 -k --keep g --keep ${FEATURE} -q gt -q ga -b 28 --loss_function logistic -f data/models/sentence.full.${WEIGHT}.${FEATURE}.vw

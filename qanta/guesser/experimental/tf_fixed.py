...
TF_DAN_WE_TMP = '/tmp/qanta/deep/tf_fixed.pickle'
TF_DAN_WE = 'tf_fixed.pickle'
GLOVE_WE = 'data/external/deep/glove.6B.300d.txt'
DEEP_FIXED_MODEL_TMP_PREFIX = '/tmp/qanta/deep/tf_fixed'
DEEP_FIXED_MODEL_TMP_DIR = '/tmp/qanta/deep'
DEEP_FIXED_MODEL_TARGET = 'tf_fixed_dir'
DEEP_FIXED_PARAMS_TARGET = 'tf_fixed_params.pickle'
...
class TFFixedLenModel:
    ...
    def build_tf_model(self):
        with tf.variable_scope(
            ...
            self.sent_vecs = tf.reshape(
                self.sent_vecs,
                shape=(
                    self.batch_size,
                    self.max_len * embedding.shape[1]
                )
            )

            self.nn_dropout_var = tf.get_variable('nn_dropout', (), dtype=tf.float32,
                                                  trainable=False)
            in_dim = self.sent_vecs.get_shape()[1]

            layer_out = self.sent_vecs
    ...
    def save(self):
        self.saver.save(self.session, safe_path(DEEP_FIXED_MODEL_TMP_PREFIX))

    def load(self):
        self.saver.restore(self.session, DEEP_FIXED_MODEL_TMP_PREFIX)
...
class FixedLenGuesser(AbstractGuesser):
    def __init__(self, dan_params=DEFAULT_FIXED_PARAMS, use_wiki=False, min_answers=2):
        super().__init__()
        self.dan_params = dan_params
        self.model = None  # type: Union[None, TFFixedLenModel]
    ...
    def train(self,
        ...
        self.model = TFFixedLenModel(self.dan_params, self.max_len, self.n_classes)
    ...
    def load(cls, directory: str) -> AbstractGuesser:
        ...
        params_path = os.path.join(directory, DEEP_FIXED_PARAMS_TARGET)
        ...
        model_path = os.path.join(directory, DEEP_FIXED_MODEL_TARGET)
        shell('cp -r {} {}'.format(model_path, safe_path(DEEP_FIXED_MODEL_TMP_DIR)))
    ...
    def save(self, directory: str) -> None:
        ...
        model_path = os.path.join(directory, DEEP_FIXED_MODEL_TARGET)
        shell('cp -r {} {}'.format(DEEP_FIXED_MODEL_TMP_DIR, safe_path(model_path)))
        we_path = os.path.join(directory, TF_DAN_WE)

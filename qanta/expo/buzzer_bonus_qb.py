...
def apply_bonus(final_score):
    try:
        more_comp = input("Assign more points to the computer? (y/n): ").strip().lower()
        if more_comp == 'y':
            pts = int(input("How many points? "))
            final_score.computer += pts
    except ValueError:
        print("Invalid input. Skipping bonus points for computer.")

    try:
        more_human = input("Assign more points to the human? (y/n): ").strip().lower()
        if more_human == 'y':
            pts = int(input("How many points? "))
            final_score.human += pts
    except ValueError:
...
def present_question_hc(
            ...
            if ss == max(question_text) and ii == len(question_text[ss].split()) - 1:
                ...
                if computer_delta == 0:
                    ...
                    """if is_correct:
                        ...
                        final_score = Score(human=human_delta, computer=question_value)
                break
                    else:
                    ...
                    if answer_check:
                        ...
                        sleep(kLONGPAUSE)
                        final_score = Score(human=human_delta, computer=10)
                        break
                    else:
            ...
            if isinstance(press, int):
                ...
                while response is None:
                    ...
                    if "+" in response:
                        ...
                        final_score = Score(human=question_value, computer=computer_delta)
                        return apply_bonus(final_score)
                        goto_bonus = True
                        break
                    elif "-" in response:
                        ...
                        if computer_delta == -5:
                            ...
                            final_score = Score(computer=computer_delta)
                            return apply_bonus(final_score)
                            goto_bonus = True
                            break
                        else:
    ...
    if human_delta == 0:
        ...
        if "+" in response:
            final_score = Score(human=10, computer=computer_delta)
            goto_bonus = True
        elif "-" in response:
            final_score = Score(human=-5, computer=computer_delta)
            goto_bonus = True
        else:
            final_score = Score(human=human_delta, computer=computer_delta)
    else:
        final_score = Score(human=human_delta, computer=computer_delta)

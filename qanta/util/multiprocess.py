...
def _multiprocess(
    func, inputs, n_cores=0, info="", progress=True, multi=True, spark=None
):
    if n_cores == 0:
    ...
    if spark is not None:
        ...
        while not result.ready():
            size = queue.qsize()
            if size > 0:
                eta = int((time.time() - start_time) / size * (total_size - size))
                eta = "{}min {}s".format(eta // 60, eta % 60)
            else:
                eta = "inf"
            if progress:
                sys.stderr.write(output.format(info, n_cores, size, total_size, eta))
            time.sleep(0.1)
        if progress:
            sys.stderr.write("\n")
        pool.close()
